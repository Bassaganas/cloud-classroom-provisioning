...
# IAM Policy for S3 access to workshop setup script bucket (attached to common EC2 IAM role)
# Get the role name from the instance profile
data "aws_iam_instance_profile" "common_profile" {
  count = (var.workshop_name == "fellowship" || var.workshop_name == "fellowship-of-the-build" || var.workshop_name == "testus_patronus") && var.common_ec2_iam_instance_profile_name != "" ? 1 : 0
  name  = var.common_ec2_iam_instance_profile_name
}

resource "aws_iam_role_policy" "ec2_sut_access" {
  count = (var.workshop_name == "fellowship" || var.workshop_name == "fellowship-of-the-build" || var.workshop_name == "testus_patronus") && length(data.aws_iam_instance_profile.common_profile) > 0 ? 1 : 0
