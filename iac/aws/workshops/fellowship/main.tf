...
module "iam_lambda" {
  ...
  instance_manager_password_secret_arn = try(data.terraform_remote_state.common.outputs.instance_manager_password_secret_arn, "")
  instance_manager_password_secret_name = try(data.terraform_remote_state.common.outputs.instance_manager_password_secret_name, "")
}
...
module "cloudfront_user_management" {
  ...
  owner            = var.owner
  workshop_name     = var.workshop_name
  domain_name      = "fellowship-of-the-build.testingfantasy.com"
...
module "cloudfront_dify_jira" {
  ...
  owner            = var.owner
  workshop_name     = var.workshop_name
  domain_name      = "dify-jira-fellowship.testingfantasy.com"
