...
def test_chat_requires_auth(api_base_url: str):
    response = requests.post(f"{api_base_url}/chat/start", json={'character': 'gandalf'}, verify=False)
    assert response.status_code == 401

def test_chat_start_and_message_flow(api_base_url: str):
    session = _login_session(api_base_url)
    started = session.post(f"{api_base_url}/chat/start", json={'character': 'sam'}, verify=False)
...
def test_chat_proposes_side_quest_when_all_done(api_base_url: str):
    session = _login_session(api_base_url)
    quests_response = session.get(f"{api_base_url}/quests/", verify=False)
    assert quests_response.status_code == 200
    quests = quests_response.json()
    assert len(quests) > 0
    for quest in quests:
        complete_response = session.put(f"{api_base_url}/quests/{quest['id']}/complete", verify=False)
        assert complete_response.status_code == 200
    started = session.post(f"{api_base_url}/chat/start", json={'character': 'gandalf'}, verify=False)
    assert started.status_code == 200
...
def test_chat_session_and_reset(api_base_url: str):
    session = _login_session(api_base_url)
    session.post(f"{api_base_url}/chat/start", json={'character': 'frodo'}, verify=False)
    session_data = session.get(f"{api_base_url}/chat/session?character=frodo", verify=False)
    assert session_data.status_code == 200
    payload = session_data.json()
    assert payload['character'] == 'frodo'
    assert isinstance(payload['messages'], list)
    reset = session.post(f"{api_base_url}/chat/reset", json={'character': 'frodo'}, verify=False)
