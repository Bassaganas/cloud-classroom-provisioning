...
WORKSHOP_PATH="iac/aws/workshops/$WORKSHOP_ROOT"
...
# Setup backend infrastructure first (shared/common and workshop)
echo "Setting up Terraform backends..."
cd iac/backend/aws

COMMON_BACKEND_BUCKET="terraform-state-classroom-shared"
COMMON_BACKEND_TABLE="terraform-locks-classroom-shared"
WORKSHOP_BACKEND_BUCKET="terraform-state-${CLASSROOM_NAME}"
WORKSHOP_BACKEND_TABLE="terraform-locks-${CLASSROOM_NAME}"
if [ "$ENVIRONMENT" = "dev" ]; then
  COMMON_STATE_KEY="classroom/shared/terraform.tfstate"
  WORKSHOP_STATE_KEY="classroom/$CLASSROOM_NAME/terraform.tfstate"
else
  COMMON_STATE_KEY="classroom/shared/${ENVIRONMENT}/terraform.tfstate"
  WORKSHOP_STATE_KEY="classroom/$CLASSROOM_NAME/${ENVIRONMENT}/terraform.tfstate"
fi
...
aws_region         = "$REGION"
state_bucket_name  = "$COMMON_BACKEND_BUCKET"
dynamodb_table_name = "$COMMON_BACKEND_TABLE"
EOF
terraform apply -auto-approve
COMMON_STATE_BUCKET=$(terraform output -raw state_bucket_name)
COMMON_STATE_TABLE=$(terraform output -raw dynamodb_table_name)

# Workshop backend (skip if only common)
if [ "$ONLY_COMMON" = false ]; then
  terraform workspace select workshop-backend >/dev/null 2>&1 || terraform workspace new workshop-backend
  cat > terraform.tfvars << EOF
aws_region         = "$REGION"
state_bucket_name  = "$WORKSHOP_BACKEND_BUCKET"
dynamodb_table_name = "$WORKSHOP_BACKEND_TABLE"
EOF
  terraform apply -auto-approve
  WORKSHOP_STATE_BUCKET=$(terraform output -raw state_bucket_name)
  WORKSHOP_STATE_TABLE=$(terraform output -raw dynamodb_table_name)
fi

echo "Backends created successfully:"
echo "  Common S3 Bucket: $COMMON_STATE_BUCKET"
echo "  Common DynamoDB Table: $COMMON_STATE_TABLE"
if [ "$ONLY_COMMON" = false ]; then
  echo "  Workshop S3 Bucket: $WORKSHOP_STATE_BUCKET"
  echo "  Workshop DynamoDB Table: $WORKSHOP_STATE_TABLE"
fi
...
# Configure common stack backend and apply
if [ "$ONLY_WORKSHOP" = false ]; then
  echo "Configuring common Terraform with remote backend..."
  cd iac/aws/common
  cat > backend.tf << EOF
terraform {
  backend "s3" {
    bucket         = "$COMMON_STATE_BUCKET"
    key            = "$COMMON_STATE_KEY"
    region         = "$REGION"
    dynamodb_table = "$COMMON_STATE_TABLE"
    encrypt        = true
  }
}
EOF
...
owner = "admin"
region = "$REGION"
workshop_name = "shared"
classroom_name = "common"
EOF
  ...
  echo "Skipping common stack (--only-workshop specified)"
fi

# Configure workshop Terraform with backend
if [ "$ONLY_COMMON" = false ]; then
  echo "Configuring workshop Terraform with remote backend..."
  cd "../../.."
  cd "$WORKSHOP_PATH"
...
terraform {
  backend "s3" {
    bucket         = "$WORKSHOP_STATE_BUCKET"
    key            = "$WORKSHOP_STATE_KEY"
    region         = "$REGION"
    dynamodb_table = "$WORKSHOP_STATE_TABLE"
    encrypt        = true
  }
}
EOF
...
workshop_name = "$WORKSHOP_ROOT"
environment = "$ENVIRONMENT"
region = "$REGION"
ec2_pool_size = $POOL_SIZE
common_state_bucket = "$COMMON_STATE_BUCKET"
common_state_key = "$COMMON_STATE_KEY"
common_state_region = "$REGION"
common_state_dynamodb_table = "$COMMON_STATE_TABLE"
EOF
  else
...
echo "Note: After deployment, use the Instance Manager URL at /ui to create and manage instances."
  ...
  if [ "$ACTION" = "destroy" ]; then
    ...
    if [ "$INSTANCE_MGR_CF_CREATED" = false ] || [ "$USER_MGMT_CF_CREATED" = false ] || [ "$DIFY_JIRA_CF_CREATED" = false ]; then
      ...
      echo "    └─ Edit: $WORKSHOP_PATH/main.tf"
      echo "    └─ Set wait_for_certificate_validation = true for the module(s) you want"
      echo "    └─ Currently:"
      INSTANCE_MGR_WAIT=$(grep -A 10 "module \"cloudfront_instance_manager\"" "$WORKSHOP_PATH/main.tf" | grep "wait_for_certificate_validation" | awk '{print $3}' || echo "unknown")
      USER_MGMT_WAIT=$(grep -A 10 "module \"cloudfront_user_management\"" "$WORKSHOP_PATH/main.tf" | grep "wait_for_certificate_validation" | awk '{print $3}' || echo "unknown")
      DIFY_JIRA_WAIT=$(grep -A 10 "module \"cloudfront_dify_jira\"" "$WORKSHOP_PATH/main.tf" | grep "wait_for_certificate_validation" | awk '{print $3}' || echo "unknown")
      ...
      echo "    └─ Run: cd $WORKSHOP_PATH && terraform apply"
  ...
  echo "Skipping workshop stack (--only-common specified)"
fi